...
class clusteringAnalyzerAll_slimmed : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
   explicit clusteringAnalyzerAll_slimmed(const edm::ParameterSet&);
private:
...
//_constructor_
clusteringAnalyzerAll_slimmed::clusteringAnalyzerAll_slimmed(const edm::ParameterSet& iConfig):
...
const reco::Candidate* clusteringAnalyzerAll_slimmed::parse_chain(const reco::Candidate* cand)
{  
...
std::string clusteringAnalyzerAll_slimmed::returnJECFile(std::string year, std::string systematicType, std::string jet_type, std::string runType)
{
...
double clusteringAnalyzerAll_slimmed::getJECUncertaintyFromSources(std::string jet_type, double pt, double eta)
{  
...
bool clusteringAnalyzerAll_slimmed::applyJERSource(std::string uncertainty_source, double eta)
{
...
bool clusteringAnalyzerAll_slimmed::isHEM(const float jet_eta, const float jet_phi)
{
...
bool clusteringAnalyzerAll_slimmed::isgoodjet(const float eta, const float NHF,const float NEMF, const size_t NumConst,const float CHF,const int CHM, const float MUF, const float CEMF,bool jetPUid, const float iJet_pt)
{
...
bool clusteringAnalyzerAll_slimmed::isgoodjet(const float eta, const float NHF,const float NEMF, const size_t NumConst,const float CHF,const int CHM, const float MUF, const float CEMF, int nfatjets)
{
...
double clusteringAnalyzerAll_slimmed::calcMPP(TLorentzVector superJetTLV )  
{
...
double clusteringAnalyzerAll_slimmed::top_pt_SF(double top_pt)
{
...
bool clusteringAnalyzerAll_slimmed::isMatchedtoSJ(std::vector<TLorentzVector> superJetTLVs, TLorentzVector candJet)
{
...
double clusteringAnalyzerAll_slimmed::calcAlphas(double q2) 
{ 
...
double clusteringAnalyzerAll_slimmed::calcFactorizWeight(LHAPDF::PDF* pdf, double id1, double id2, double x1, double x2, double q2, int up_or_dn) 
{
    double k2;
    if ( up_or_dn ==  1 )
        k2 = 4; // 2*q ==> 4*q2
...
double clusteringAnalyzerAll_slimmed::calcRenormWeight(double q2, int up_or_dn, int nQCD) 
{ 
...
bool clusteringAnalyzerAll_slimmed::fillSJVars(std::map<std::string, float> &treeVars, std::vector<fastjet::PseudoJet> iSJ, int nSuperJets )
{
...
void clusteringAnalyzerAll_slimmed::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
DEFINE_FWK_MODULE(clusteringAnalyzerAll_slimmed);
//_bottom_
