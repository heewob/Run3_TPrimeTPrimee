// -*- C++ -*-
//
// Package:    genPartFilter_TPrime/genPartFilter_TPrime
// Class:      genPartFilter_TPrime
// 
/**\class genPartFilter_TPrime genPartFilter_TPrime.cc genPartFilter_TPrime/genPartFilter_TPrime/plugins/genPartFilter_TPrime.cc
...
class genPartFilter_TPrime : public edm::stream::EDFilter<> {
   public:
      explicit genPartFilter_TPrime(const edm::ParameterSet&);
      ~genPartFilter_TPrime();
...
//
genPartFilter_TPrime::genPartFilter_TPrime(const edm::ParameterSet& iConfig)
{
...
genPartFilter_TPrime::~genPartFilter_TPrime()
{
...
const reco::Candidate* genPartFilter_TPrime::parse_chain(const reco::Candidate* cand)
{  
...
bool
genPartFilter_TPrime::filter(edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   //using namespace edm;
...
//want only fully hadronic events_
   ...
   { 
if ((abs(iG->pdgId()) == 24) && (abs(iG->mother()->pdgId()) == Tprime_pdgid)) nW_++;
if ((abs(iG->pdgId()) == 5) && (abs(iG->mother()->pdgId()) == Tprime_pdgid)) nSuub_++;
if ((abs(iG->pdgId()) == 23) && (abs(iG->mother()->pdgId()) == Tprime_pdgid)) nZ_++;
if ((abs(iG->pdgId()) == 6) && (abs(iG->mother()->pdgId()) == Tprime_pdgid)) ntop_++;
if ((abs(iG->pdgId()) == 25) && (abs(iG->mother()->pdgId()) == Tprime_pdgid)) nH_++;
...
if ((abs(iG->pdgId())==24)&&((abs(iG->mother()->pdgId())==Tprime_pdgid)) )
      ...
      */
      //else if  ((abs(iG->pdgId()) == Tprime_pdgid) && (iG->isLastCopy())) nChi++;
...
void
genPartFilter_TPrime::beginStream(edm::StreamID)
{
}

// ------------ method called once each stream after processing all runs, lumis and events  ------------
void
genPartFilter_TPrime::endStream() {
...
/*
void
genPartFilter_TPrime::beginRun(edm::Run const&, edm::EventSetup const&)
{ 
}
*/
...
/*
void
genPartFilter_TPrime::beginLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
/*
void
genPartFilter_TPrime::endLuminosityBlock(edm::LuminosityBlock const&, edm::EventSetup const&)
{
}
*/
...
void
genPartFilter_TPrime::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(genPartFilter_TPrime);
