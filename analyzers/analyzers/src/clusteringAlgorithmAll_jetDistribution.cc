...
class clusteringAnalyzerAll_jetDistribution : public edm::one::EDAnalyzer<edm::one::SharedResources> {
public:
   explicit clusteringAnalyzerAll_jetDistribution(const edm::ParameterSet&);
private:
...
//_constructor_
clusteringAnalyzerAll_jetDistribution::clusteringAnalyzerAll_jetDistribution(const edm::ParameterSet& iConfig):
...

{
   ...
   */
   ...
   tree->Branch("L_perpendicular", &L_perpendicular, "L_perpendicualr/D");
   tree->Branch("L_ratio", &L_ratio, "L_ratio/D");
...
const reco::Candidate* clusteringAnalyzerAll_jetDistribution::parse_chain(const reco::Candidate* cand)
{  
...
std::string clusteringAnalyzerAll_jetDistribution::returnJECFile(std::string year, std::string systematicType, std::string jet_type, std::string runType)
{
...
double clusteringAnalyzerAll_jetDistribution::getJECUncertaintyFromSources(std::string jet_type, double pt, double eta)
{  
...
bool clusteringAnalyzerAll_jetDistribution::applyJERSource(std::string uncertainty_source, double eta)
{
...
bool clusteringAnalyzerAll_jetDistribution::isHEM(const float jet_eta, const float jet_phi)
{
...
bool clusteringAnalyzerAll_jetDistribution::isgoodjet(const float eta, const float NHF,const float NEMF, const size_t NumConst,const float CHF,const int CHM, const float MUF, const float CEMF,bool jetPUid, const float iJet_pt)
{
...
bool clusteringAnalyzerAll_jetDistribution::isgoodjet(const float eta, const float NHF,const float NEMF, const size_t NumConst,const float CHF,const int CHM, const float MUF, const float CEMF, int nfatjets)
{
...
double clusteringAnalyzerAll_jetDistribution::calcMPP(TLorentzVector superJetTLV )  
{
...
double clusteringAnalyzerAll_jetDistribution::top_pt_SF(double top_pt)
{
...
bool clusteringAnalyzerAll_jetDistribution::isMatchedtoSJ(std::vector<TLorentzVector> superJetTLVs, TLorentzVector candJet)
{
...
double clusteringAnalyzerAll_jetDistribution::calcAlphas(double q2) 
{ 
...
double clusteringAnalyzerAll_jetDistribution::calcFactorizWeight(LHAPDF::PDF* pdf, double id1, double id2, double x1, double x2, double q2, int up_or_dn) 
{
    double k2;
    if ( up_or_dn ==  1 )
        k2 = 4; // 2*q ==> 4*q2
...
double clusteringAnalyzerAll_jetDistribution::calcRenormWeight(double q2, int up_or_dn, int nQCD) 
{ 
...
bool clusteringAnalyzerAll_jetDistribution::fillSJVars(std::map<std::string, float> &treeVars, std::vector<fastjet::PseudoJet> iSJ, int nSuperJets )
{
...
void clusteringAnalyzerAll_jetDistribution::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
...
////////////////////////////////////////calculate the jet distribution////////////////////////////////////////
//make sure the thrust axis is a unit vector
math::XYZVector n1 = thrustAxis.unit();
// Get a perpendicular vector in transverse plane
math::XYZVector zAxis(0, 0, 1);
math::XYZVector n2 = n1.Cross(zAxis);

// Handle edge case where thrust axis is aligned with z-axis
if (n2.R() < 1e-6) {
    n2 = math::XYZVector(1, 0, 0);  // fallback
}
n2 = n2.unit();  // Normalize
...
L_ratio = -1.0;
/*
///////////////////////////////////////////sanity check thrist axis//////////////////////////////////////////////
// 1. Get the gen-level T' and T̄' 4-momenta
const reco::GenParticle* genT = nullptr;
const reco::GenParticle* genTbar = nullptr;

for (const auto& gen : *genParticlesHandle) {
    if (std::abs(gen.pdgId()) == 8000001 && gen.status() == 22) { // or 23 depending on how your T' is defined
        if (gen.pdgId() > 0) genT = &gen;
        else genTbar = &gen;
    }
}
...
// 2. Sanity check
if (!genT || !genTbar) {
    edm::LogWarning("ThrustCheck") << "Tprime or anti-Tprime not found";
    return;
}

// 3. Compute boost to CoM frame
math::XYZTLorentzVector pTot = genT->p4() + genTbar->p4();
ROOT::Math::Boost boostToCoM(-pTot.BoostToCM());

// 4. Boost gen Tprime and T̄prime to CoM
math::XYZTLorentzVector genT_boosted = boostToCoM(genT->p4());
math::XYZTLorentzVector genTbar_boosted = boostToCoM(genTbar->p4());

// 5. Construct T'-T' axis in CoM frame
math::XYZVector ttbar_axis = (genT_boosted.Vect() - genTbar_boosted.Vect()).Unit();

// 6. Boost thrust axis to CoM frame as a spatial vector
// Note: you treat thrustAxis as a momentum 4-vector with E=|p| to apply boost
math::XYZTLorentzVector thrust_4vec(thrustAxis.x(), thrustAxis.y(), thrustAxis.z(), thrustAxis.R());
math::XYZVector thrust_axis_com = boostToCoM(thrust_4vec).Vect().Unit();

// 7. Compare angle
double cosTheta = thrust_axis_com.Dot(ttbar_axis); // in [-1, 1]
cosTheta = std::abs(cosTheta); // remove sign, if needed

// 8. Save to TTree
thrust_alignment_cosTheta = cosTheta;
///////////////////////////////////////////////////////////////////////////////////////////////////////////
*/
// Loop over CoM AK8 jets fastjet::PseudoJet collection
for (const auto& jet : jetsFJ_jet0) {
    math::XYZVector pjet(jet.px(), jet.py(), jet.pz());  // Convert to XYZVector

    L_parallel     += std::abs(pjet.Dot(n1));
    L_perpendicular += std::abs(pjet.Dot(n2));
}

if (L_parallel > 0.0) {
    L_ratio = L_perpendicular / L_parallel;
} else {
    edm::LogWarning("ThrustAnalysis") << "L_parallel is zero; skipping ratio computation.";
    L_ratio = -1.0;  //invalid ratio 
}
...
DEFINE_FWK_MODULE(clusteringAnalyzerAll_jetDistribution);
//_bottom_
